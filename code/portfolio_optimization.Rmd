...
for(stock in names(stocks_r) ){
  stocks_r[,stock] <- periodReturn(stocks[,stock], 
...
# proportion to apply to each stock
...
# why does constriant turn to = instead of >=
model$sense <- c('=', '>=')
...
{
  model$rhs <- c(1, min_returns[i])
  result <- gurobi(model, list())
  ef_return[i] = sum(result$x * returns)
  ef_risk[i] = sqrt(result$objval)
