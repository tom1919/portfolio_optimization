...
# Contraint mtx: 
# sum of weights = 1
# weighted sum of returns >= .002503 
# weight >= 0 (can't be negative)
# (0.05% each day for 5 days = 1.0005^5 - 1 = 0.002503)?? or just .0005
model$A <- matrix(c(rep(1,ncol(cov_mtx)),
                    returns),
                  nrow=2,byrow=T)
model$A <- rbind(model$A, diag(rep(1, ncol(cov_mtx))))
# model$A <- rbind(model$A, diag(length(returns)))
